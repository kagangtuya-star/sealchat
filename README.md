...
<img width="1632" height="872" alt="PixPin_2026-08-11_14-53-31" src="https://github.com/user-attachments/assets/e307c10f-b057-459a-a174-7892f86a9a97" />
...
- 分类开关：`storage.s3.attachmentsEnabled`（附件/图片）、`storage.s3.audioEnabled`（音频）、`storage.s3.fontsEnabled`（平台字体原文件/分片/manifest）、`storage.s3.theaterEnabled`（小剧场视觉与音频资源；省略时继承对应分类开关）。
...
- 迁移工具：管理端“迁移到 S3”支持图片、音频、小剧场视觉资源分别迁移，建议先“模拟运行（dryRun）”。小剧场音频仍归入音频迁移。
