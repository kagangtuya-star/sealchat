...
func Init(config *utils.AppConfig, uiStatic fs.FS) error {
	...
	audio.Get("/library/settings", AudioLibrarySettingsGet)
	audio.Put("/library/settings", AudioLibrarySettingsPut)
	audio.Get("/library/s3/prefixes", AudioLibraryS3Prefixes)
	audio.Get("/library/assets", AudioLibraryS3Assets)
	audio.Get("/library/assets/selector", AudioLibraryS3SelectorAssets)
	audio.Post("/library/assets/resolve", AudioLibraryResolveAssets)
	audio.Post("/library/play-token", AudioLibraryPlayToken)
	audioAdminLibrary := audio.Group("/library", AudioWorkbenchMiddleware)
	audioAdminLibrary.Post("/upload", AudioLibraryUpload)
	audioAdminLibrary.Post("/folders", AudioLibraryFolderCreate)
	audioAdminLibrary.Delete("/folders", AudioLibraryFolderDelete)
	audioAdminLibrary.Patch("/assets", AudioLibraryAssetUpdate)
	audioAdminLibrary.Delete("/assets", AudioLibraryAssetDelete)
	...
	v1AuthAdmin.Get("/admin/character-card-templates", AdminCharacterCardTemplateList)
	v1AuthAdmin.Get("/admin/character-card-templates/:id", AdminCharacterCardTemplateGet)
	v1AuthAdmin.Post("/admin/character-card-templates", AdminCharacterCardTemplateCreate)
	v1AuthAdmin.Patch("/admin/character-card-templates/:id", AdminCharacterCardTemplateUpdate)
	v1AuthAdmin.Post("/admin/character-card-templates/:id/enable", AdminCharacterCardTemplateEnable)
	v1AuthAdmin.Post("/admin/character-card-templates/:id/disable", AdminCharacterCardTemplateDisable)
	v1AuthAdmin.Delete("/admin/character-card-templates/:id", AdminCharacterCardTemplateDelete)
	...
	v1AuthAdmin.Get("/admin/perf/message-pipeline", AdminPerfMessagePipeline)
	v1AuthAdmin.Post("/admin/perf/message-pipeline/reset", AdminPerfMessagePipelineReset)
	v1AuthAdmin.Post("/admin/perf/trace-session/start", AdminPerfTraceSessionStart)
	v1AuthAdmin.Post("/admin/perf/trace-session/stop", AdminPerfTraceSessionStop)
