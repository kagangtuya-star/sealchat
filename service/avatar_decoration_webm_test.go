...
func TestNormalizeAvatarDecorationAllowsPlaybackRateOutsidePreviousRange(t *testing.T) {
	initTestDB(t)
	db := model.GetDB()

	userID := "user-avatar-decoration-webm-large-rate"
	attachmentID := "attachment-avatar-decoration-webm-large-rate"
	if err := db.Create(&model.AttachmentModel{
		StringPKBaseModel: model.StringPKBaseModel{ID: attachmentID},
		UserID:            userID,
		Filename:          "decoration.webm",
	...
	if err != nil {
		t.Fatalf("expected larger playback rate to be accepted, got error: %v", err)
	}
	if decoration == nil {
		t.Fatalf("expected normalized avatar decoration")
	}
	if decoration.Settings.PlaybackRate != 3 {
		t.Fatalf("expected playback rate to be preserved, got %v", decoration.Settings.PlaybackRate)
	}
}

func TestNormalizeAvatarDecorationAllowsScaleOutsidePreviousRange(t *testing.T) {
	initTestDB(t)
	db := model.GetDB()

	userID := "user-avatar-decoration-large-scale"
	attachmentID := "attachment-avatar-decoration-large-scale"
	if err := db.Create(&model.AttachmentModel{
		StringPKBaseModel: model.StringPKBaseModel{ID: attachmentID},
		UserID:            userID,
		...
		t.Fatalf("create png attachment failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("expected larger scale to be accepted, got error: %v", err)
	}
	if decoration == nil {
		t.Fatalf("expected normalized avatar decoration")
	}
	if decoration.Settings.Scale != 2.4 {
		t.Fatalf("expected scale to be preserved, got %v", decoration.Settings.Scale)
	}
}
