...
state = dispatchTheaterEditorCommand(state, { type: 'set-layer-property', target: { kind: 'content' }, property: 'fontAssetId', value: 'platform-font-1' })
assert.equal(state.draft.dialogue.content.fontAssetId, 'platform-font-1', 'content font asset id must update')
state = dispatchTheaterEditorCommand(state, { type: 'set-layer-property', target: { kind: 'content' }, property: 'fontAssetId', value: '' })
assert.equal(state.draft.dialogue.content.fontAssetId, undefined, 'clearing content font asset id must remove field')
state = dispatchTheaterEditorCommand(state, { type: 'set-layer-property', target: { kind: 'speaker' }, property: 'fontAssetId', value: 'platform-font-2' })
assert.equal(state.draft.dialogue.speaker.fontAssetId, 'platform-font-2', 'speaker font asset id must update')
state = dispatchTheaterEditorCommand(state, { type: 'set-layer-property', target: { kind: 'speaker' }, property: 'fontAssetId', value: '' })
assert.equal(state.draft.dialogue.speaker.fontAssetId, undefined, 'clearing speaker font asset id must remove field')
