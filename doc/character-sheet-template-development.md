...
- `力量{力量} 敏捷{敏捷} 体质{体质}`
- `侦查{侦查} 聆听{聆听} 闪避{闪避}`
- `得意器术{$忍神.specialtySwitches.器术}`

说明：

- `{生命值}` 优先读取人物卡 attrs 根级键名为 `生命值` 的值
- `{对象.子项.属性}` 可读取嵌套 JSON，例如 `{$忍神.specialtySwitches.器术}`
- 单段占位符在根级键不存在时，会按 JSON 原始属性顺序深度优先查找嵌套数据中首个同名键；例如 `{器术}` 可读取 `$忍神.specialtySwitches.器术`
- 多处存在同名键时，请使用完整点路径，避免数据结构调整后命中不同值
- 点路径不存在时不会自动降级为末段键名查找
