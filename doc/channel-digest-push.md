...
系统不再围绕“某个用户是否未读”来触发提醒，而是围绕“某个频道在一个时间周期内是否出现了消息、但整体活跃度仍然偏低”来生成摘要，并通过以下两种方式对外提供：
...
3. 发消息的成员也会计入该周期活跃人数
4. 不依赖 presence 心跳，不取在线快照

支持两种模式：

1. `channel_member_count`
说明：阈值动态等于当前频道成员数上限，适合“频道成员几乎都没来”的场景。

2. `fixed`
说明：手动指定固定活跃人数上限，默认采用此模式。
